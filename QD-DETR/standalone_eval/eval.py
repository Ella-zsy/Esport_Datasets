...
def compute_mr_ap(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10),
                  max_gt_windows=None, max_pred_windows= 40, num_workers=8, chunksize=50):
    iou_thds = [float(f"{e:.2f}") for e in iou_thds]
    # 将 iou_thds 中的数字转换为字符串
    #iou_thds_str = [f"{e:.2f}" for e in iou_thds]
    ...
    if np.isscalar(ap_thds):
        ap_thds = [ap_thds]  # 将单一浮点数变为列表
...
def get_data_by_range(submission, ground_truth, len_range):
    ...
    if min_l == 0 and max_l == 2000:  # min and max l in dataset
