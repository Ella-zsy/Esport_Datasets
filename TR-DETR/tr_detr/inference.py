...
from tr_detr.config import TestOptions
from tr_detr.model import build_model
from tr_detr.span_utils import span_cxw_to_xx
from tr_detr.start_end_dataset import StartEndDataset, start_end_collate, prepare_batch_inputs
from tr_detr.start_end_dataset_audio import \
    StartEndDataset_audio, start_end_collate_audio, prepare_batch_inputs_audio
from tr_detr.postprocessing_tr_detr import PostProcessorDETR
    ...
    for batch in tqdm(eval_loader, desc="compute st ed scores"):
        ...
        for meta, pred in zip(query_meta, preds):
            ...
            for i in range(20):
                ...
                cur_label = torch.where(cur_label > cur_label.median(), 1.0, .0).cuda()
...
def compute_mr_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None):
    ...
    post_processor = PostProcessorDETR(
        clip_length=2, min_ts_val=0, max_ts_val=2000,#150
        min_w_l=2, max_w_l=2000, move_window_method="left",#150
